...
function renderParams(sp){
  ...
  if(!has)queryList.innerHTML='<div class="query-empty">No query params</div>';
}
...
function addParamRow(key,val){
  ...
  btn.addEventListener('click',function(){row.remove();buildCurrentURL();if(!queryList.querySelector('.query-row'))queryList.innerHTML='<div class="query-empty">No query params</div>';});
